...
def info_theoretic_measures():
    ...
    for t in range(nodes_n):
        ...
        if 'TE' in traj.par.network_inference.algorithm:
            ...
            if debug_mode:
                print('TE_omnibus_empirical = {0}'.format(TE_omnibus_empirical[t]))
                print('TE_complete_empirical = {0}'.format(TE_complete_empirical[:, :, t]))
    ...
    df.loc[run_name, 'TE_omnibus_theoretical_inferred_vars'] = (
        TE_omnibus_theoretical_inferred_vars)
    df.loc[run_name, 'TE_complete_theoretical_inferred_vars'] = (
        TE_complete_theoretical_inferred_vars)
    df.loc[run_name, 'TE_apparent_theoretical_inferred_vars'] = (
        TE_apparent_theoretical_inferred_vars)
    df.loc[run_name, 'AIS_theoretical_inferred_vars'] = (
        AIS_theoretical_inferred_vars)
    # causal vars
    df.loc[run_name, 'TE_omnibus_theoretical_causal_vars'] = (
        TE_omnibus_theoretical_causal_vars)
    df.loc[run_name, 'TE_complete_theoretical_causal_vars'] = (
        TE_complete_theoretical_causal_vars)
    df.loc[run_name, 'TE_apparent_theoretical_causal_vars'] = (
        TE_apparent_theoretical_causal_vars)
    df.loc[run_name, 'AIS_theoretical_causal_vars'] = (
...
for run_name in traj.f_get_run_names():
    ...
    # Get adjacency, delay, and coupling matrices
    if release_results_class:
...
#     if len(argv) >= 6:
#         print('\nWARNING: too many parameters provided.\n')
#     postprocess(traj_dir, fdr, debug_mode, save_results)
# 
# def postprocess(traj_dir, fdr=False, debug_mode=False, save_results=True):
