...
# Assemble full paths
traj_fullpath_list = [os.path.join(base_dir, dir_name, traj_filename) for dir_name in traj_dir_names]
...
for traj_fullpath in traj_fullpath_list:
    if os.path.exists(traj_fullpath):
        ...
        raise ValueError('ERROR: Trajectory file not found: {0}'.format(traj_fullpath))
...
for traj_i in range(len(traj_fullpath_list)):
    ...
    if not trajectories[traj_i].f_is_completed():
        raise ValueError('ERROR: Trajectory not completed or not assembled after single target analysis: {0}'.format(traj_fullpath_list[traj_i]))
...
if not os.path.isdir(output_dir):
    raise ValueError("ERROR: Output directory not found: {0}".format(output_dir))
# Convert to full path
output_dir = os.path.abspath(output_dir)
# Add time stamp (the final '' is to make sure there is a trailing slash)
output_dir = os.path.join(output_dir, datetime.now().strftime("%Y_%m_%d_%Hh%Mm%Ss") + '_merged', '')
