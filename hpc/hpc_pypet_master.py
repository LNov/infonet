...
def main():
    ...
    traj.f_add_parameter('network_inference.n_perm_max_seq', 500, comment='')
    ...
    explore_dict = cartesian_product(
        {
            'repetition_i': np.arange(5, 10, 1).tolist(),
            'topology.initial.nodes_n': np.arange(100, 100+1, 30).tolist(),
            'node_dynamics.samples_n': (10 ** np.arange(3, 3+0.1, 1)).round().astype(int).tolist()
            ,
